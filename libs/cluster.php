...
function groupSimilarNews($threshold = 0.90)
{
    ...
    try {
        ...
        $titles = [];
        $idMapping = [];
        // populating $titles array (title and description ($processedTitle)) and $idMapping[$processedTitle]
        ...
        // computing tfidf of $titles
        echo "computing tfidf \n";
        $tfidfVectors = computeTFIDF($titles);


        $groupedTitles = [];     // from items that already have group_uuid
        $ungroupedTitles = [];   // from items that have group_uuid == null
        ...
        // ================== Newly Added Part ======================= //
        // this is for finding the best matched group
        // 🧠 Group items by group_uuid
        $groupedMap = []; // group_uuid => array of titles
        ...
        $used = [];
        // Try to match ungrouped items to existing grouped items
        foreach ($ungroupedTitles as $index => $item) {
            ...
            // ✅ Only compare representative (centroid) per group
            ...
            // No match with grouped items → now try to group with other ungrouped items
            echo '❌ Title => ' . $index . '/' . count($ungroupedTitles) . ' : Not Matched with an Existing Group!' . "\n";
            $group = [$item];
            $used[] = $index;

            foreach ($ungroupedTitles as $subIndex => $otherItem) {
                if ($index === $subIndex || in_array($subIndex, $used)) {
                    continue;
                }

                $similarity = computeCombinedSimilarity($item, $otherItem, $tfidfVectors);
            ...
            if (count($group) > 1) {
                $guuid = uniqid(); // 🔥 generate new uuid for this group
                ...
                foreach ($group as $groupItem) {
                    ...
                    if ($result) {
                        echo "✅ New group_uuid '$guuid' set for ID $id\n";
                    } else {
                        echo "❌ Failed to update group_uuid for ID $id\n";
                    }
                }
            } else {
                echo '❌ Title => ' . '*/' . count($ungroupedTitles) . ' : No Match With a Other Titles Too!' . "\n";
            }
        }
